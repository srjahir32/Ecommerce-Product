	...
	<div class="row checkout_form_box">
		...
		<div class="col-lg-7 checkout_part_1">
			...
			<div class="product_title">
				...
				<i id="p-quantity-minus"
								class="fas fa-minus-circle quantity_btn quantity_btn_disable"></i></span> <span>
								@if($cartproduct[0]['order_limit'] > 0)
									@if($cartproduct[0]['order_limit'] > $cartproduct[0]['stock'])	
									<input type="text" id="p_quantity" name="p_quantity" max="{{$cartproduct[0]['stock']}}" value="1" disabled />
									@else
									<input type="text" id="p_quantity" name="p_quantity" max="{{$cartproduct[0]['order_limit']}}" value="1" disabled />
									@endif
								@else
								<input type="text" id="p_quantity" name="p_quantity" max="{{$cartproduct[0]['stock']}}" value="1" disabled />
								@endif
							</span> <span><i id="p-quantity-plus" class="fas fa-plus-circle  quantity_btn"></i></span> </div>

				
				@if($cartproduct[0]['stock'] > 0)
					<button class="add_to_cart" style="display: none;">Add to Cart <i class="fas fa-chevron-right"></i></button>
				@else
					<div class="outofstock_box">
						<p>Out Of Stock</p>
					</div>
				@endif
				...
				<div class="product-description">
					...
					<p> @php $allowed = '
						...
						<div><span><pre><p><br><hr><hgroup><h1><h2><h3><h4><h5><h6><ul><ol><li><dl><dt><dd><strong><b><i><u><img><a><abbr><address><blockquote><area><audio><video><form><fieldset><label><input><textarea><caption><table><tbody><td><tfoot><th><thead><tr><iframe>';
...
<!-- 
					...
					<fieldset id="check_form_2">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" placeholder="Mobile Number" id="mobile" name="mobile"> </div>
					...
					@if($cartproduct[0]['payment_type'] == 'Payfull')
						...
						<div class="form-group card_info">
							...
							<div class="cardexpiry_info">
								...
								<input type="text" class="form-control" id="cardcvc" maxlength="3" placeholder="CVC" name="cardcvc">
	...
	var $final_qty = $input.val();
	var $final_price = $price.text();
	var $final_total = $price.text();
	...
	function validform1() {
		...
		if($("#email").val() === "") {
			...
			if(!regex.test($("#email").val())) {
				...
				$("#email").addClass("form-control-error");
				$("#email").after("<span class='error'>You should not use special characters.</span>");
	...
	function validform2() {
		...
		if($("#mobile").val() === "" || $("#mobile").val().length > 13 || $("#mobile").val().length < 13) {
	...
	// 	var num = $("#cardcvc").val().replace("\d{3}", '');
	// 	$("#cardcvc").val(num);
	// });
	...
	$(".submit").click(function() {
		...
		if($('#payment_type').val() == 'Payfull') {
			...
			if(validform3()) {
				...
				product_total_price = $final_total;
