        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <h3 class="topbar_title mb-0"><img class="invoice_top_icon topbar_title_icon"
                        src="{{ asset('admin/assets/img/invoices/icon-sales.svg') }}" alt=""><span>{{count($orderlist['data'])}} Orders</span></h3>
            </div>
        ...
        <table class="products_table pending_order_table table">
            ...
            <tbody>
                ...
                <tr class="order_{{$order['id']}}">
                    <td><a href="generate_invoice/{{$order['id']}}"><img src="{{ asset('admin/assets/img/invoices/invoice_icon.svg') }}" alt=""></a></td>
                    <td>{{$order['id']}}</td>
                    <td><span class="currency_symbol">{{explode('-', $order['currency'])[0]}}</span>{{$order['total']}}</td>
                    <td class="panding_order_list_name themecolor" data-toggle="modal" data-target="#customerDetailModal" onClick="customerDetail({{$order['id']}})">{{$order['customer_name']}}</td>
                    <td>
